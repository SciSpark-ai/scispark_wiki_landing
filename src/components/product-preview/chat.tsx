...
export function ProductChat({mode, setMode, conversationId, setConversationId, openPaper, navigate}: {
  ...
  function submit() {
    ...
    addConversation({id, question:question.trim(), mode, interest:state.interest}); setConversationId(id); setQuestion('');
  }
  return <div className={`p-page p-chat-workspace ${conversation ? 'has-conversation' : ''}`}>
    ...
    {conversation && <div className="p-conversation"><div className="p-user-message">{conversation.question}</div><article className="p-assistant-message"><span className="p-chip">{t('prepared')}</span><h4>{conversation.mode === 'review' ? t('report') : topics(`${interest}Name`)}</h4><p>{conversation.mode === 'review' ? t('reportBody') : topics(`${interest}Note`)}</p>
      <div className="p-chat-sources"><span>{t('sources')}</span>{ids.map((id,index) => <button key={id} className="p-link" onClick={() => openPaper(id)}>{index+1}. {titles(`${id}Title`)}</button>)}</div>
    ...
    <form className="p-composer" onSubmit={event => {event.preventDefault();submit();}}><textarea data-tour="chat-input" aria-label={p('askSparky')} placeholder={p('askSparky')} value={question} maxLength={5000} onChange={event => setQuestion(event.target.value)} /><button className="p-icon" data-tour="chat-send" disabled={!question.trim()} aria-label={t('send')}><ArrowUp size={20}/></button></form>
